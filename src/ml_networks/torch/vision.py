...
class Encoder(BaseModule):
    ...
    def __init__(
        ...
        if self._is_vit:
            # ViT encoder returns the CLS token of shape (B, d_model); skip the convolutional reshape.
...
class _ViTEncoderBlock(nn.Module):
    ...
    Follows the DETR convention (https://github.com/gokul-pv/DetectionTransformer):
    at every layer, the spatial positional embedding is added to the query and key tensors
    of the self-attention, while the value tensor is left unmodified.
    """
...
class _ViTDecoderBlock(nn.Module):
    ...
    The block performs (pre-norm) cross-attention where the queries are the learnable patch
    tokens and the key/value come from the projected CLS representation, followed by a residual
    MLP block.
    """
...
class ViT(nn.Module):
    ...
    ``(B, 1, d_model)`` and reconstructs an image. The CLS token is projected to be used as the
    key/value of cross-attention. A fixed set of ``P = (H // p) * (W // p)`` learnable query
    tokens interacts with this representation through several cross-attention layers with
    residual MLP blocks. Each query is then linearly projected to ``p * p * C`` pixels and
    rearranged into a ``(B, C, H, W)`` image.

    Parameters
    ----------
    in_shape : tuple[int, int, int]
        Shape of input tensor (encoder mode). Ignored except for ``patch_embed`` configuration
        in encoder mode. In decoder mode the CLS-token dimension is taken from ``cfg``.
    cfg : ViTConfig
        Network configuration.
    obs_shape : tuple[int, int, int] | None
        Shape of output tensor for decoder mode. When ``None`` the module is an encoder.
    ...
    def _build_decoder(self) -> None:
        ...
        # Learnable patch queries (P, d_model). One query per output patch.
        ...
        # Final linear projection: each query -> p*p*C pixels.
    ...
    def forward(self, x: torch.Tensor, return_cls_token: bool = False) -> torch.Tensor:
        ...
        ----------
        x : torch.Tensor
            In encoder mode: image tensor of shape ``(B, C, H, W)``.
            In decoder mode: CLS token of shape ``(B, d_model)`` or ``(B, 1, d_model)``.
